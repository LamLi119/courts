...
import { buildVenueStaticBodyHtml } from './venueSeoContent.js';
...
export function getVenueDescription(venue, lang = 'en') {
  // Prefer rich location prose for meta/OG (mirrors src/utils/venueContent flattenVenueSeoForMeta).
  const name = cleanText(venue.name) || 'Venue';
  const sport = getSportTypeLabel(venue, lang);
  const mtr = cleanText(venue.mtrStation);
  ...
  const amenities = Array.isArray(venue.amenities)
    ? venue.amenities.map((a) => cleanText(a)).filter(Boolean).slice(0, 8)
    : [];

  const parts = [];
  if (lang === 'zh') {
    parts.push(
      `${name}位於香港，提供${sport}場地服務。`
      + (addr ? `場館地址為${addr}。` : '')
      + (mtr
        ? `鄰近港鐵${mtr}${exit ? `（${exit}出口）` : ''}`
          + (walkingDistance != null && walkingDistance > 0 ? `，步行約${walkingDistance}分鐘` : '')
          + '。'
        : ''),
    );
    const facts = [];
    if (courtCount != null && courtCount > 0) facts.push(`場內約有${courtCount}個球場`);
    if (ceilingHeight != null && ceilingHeight > 0) facts.push(`網高約${ceilingHeight}米`);
    if (startingPrice != null && startingPrice > 0) facts.push(`起步價約每小時$${startingPrice}`);
    if (facts.length) parts.push(`關於場地規格：${facts.join('；')}。`);
    if (amenities.length) parts.push(`常用設施包括：${amenities.join('、')}。`);
  } else {
    parts.push(
      `${name} is a ${sport.toLowerCase()} venue in Hong Kong.`
      + (addr ? ` The listed address is ${addr}.` : '')
      + (mtr
        ? ` It is near ${mtr} MTR`
          + (exit ? ` (Exit ${exit})` : '')
          + (walkingDistance != null && walkingDistance > 0 ? `, about a ${walkingDistance}-minute walk` : '')
          + '.'
        : ''),
    );
    const facts = [];
    if (courtCount != null && courtCount > 0) facts.push(`approximately ${courtCount} court${courtCount === 1 ? '' : 's'}`);
    if (ceilingHeight != null && ceilingHeight > 0) facts.push(`around ${ceilingHeight}m ceiling height`);
    if (startingPrice != null && startingPrice > 0) facts.push(`starting from about HK$${startingPrice} per hour`);
    if (facts.length) parts.push(`Venue details on Courts include ${facts.join(', ')}.`);
    if (amenities.length) parts.push(`Listed amenities include ${amenities.join(', ')}.`);
  }

  let text = parts.join(' ').replace(/\s+/g, ' ').trim();
  if (text.length > 320) {
    const cut = text.slice(0, 319);
    const safer = cut.replace(/\s+\S*$/, '');
    text = `${(safer || cut).trimEnd()}…`;
  }
  if (text) return text;
...
export function buildVenueOgMeta(venue, opts) {
  ...
  // AggregateRating only when real review data exists — never fabricate.
  ...
  return {
    ...
    staticBodyHtml: buildVenueStaticBodyHtml(venue, { pageUrl, lang }),
  };
}
...
export function injectPageSeoIntoHtml(html, meta) {
  ...
  out = out.replace(/<article[^>]*data-seo-static="1"[^>]*>[\s\S]*?<\/article>/gi, '');
  ...
  const rootInner = [staticBodyHtml, noscriptHtml].filter(Boolean).join('\n');
  if (rootInner) {
    out = out.replace(/<div id="root"><\/div>/i, `<div id="root">${rootInner}</div>`);
  }
