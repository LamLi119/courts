...
function toSentenceList(values, fallback = '') {
  const items = (values || []).map((v) => cleanText(v)).filter(Boolean);
...
function formatOperatingHours(raw) {
  ...
  for (const key of Object.keys(dayLabels)) {
    ...
    const slots = Array.isArray(day.slots) ? day.slots.map((slot) => Array.isArray(slot) ? slot.join('-') : '').filter(Boolean) : [];
...
function venueMarkdown(venue, baseUrl) {
  ...
  const description = cleanText(venue?.description) || `${cleanText(venue?.name)} sports venue in Hong Kong.`;
  ...
  const body = [
    ...
    cleanText(venue?.mtrStation) ? `- MTR: ${cleanText(venue.mtrStation)}${cleanText(venue?.mtrExit) ? ` (${cleanText(venue.mtrExit)})` : ''}` : '',
    venue?.walkingDistance != null && venue.walkingDistance !== '' ? `- Walking distance: ${venue.walkingDistance} min` : '',
    sports.length ? `- Sports: ${sports.join(', ')}` : '',
    venue?.startingPrice != null && venue.startingPrice !== '' ? `- Starting price: HKD ${venue.startingPrice}/hr` : '',
    venue?.court_count != null && venue.court_count !== '' ? `- Court count: ${venue.court_count}` : '',
...
function sportMarkdown(sport, venues, baseUrl) {
  ...
  const supportedVenues = (venues || []).filter((venue) =>
    Array.isArray(venue?.sport_data) && venue.sport_data.some((item) => cleanText(item?.slug) === slug)
  );
  ...
  const body = [
    ...
    supportedVenues.length
      ? `## Venues\n${supportedVenues.map((venue) => `- [${cleanText(venue.name)}](${baseUrl}/venues/${slugifyVenueName(venue.name)})`).join('\n')}\n`
      : '',
  ].filter(Boolean).join('\n');
  ...
  const files = [
    ...
    },
    {
      ...
      content: [
        '# Venues',
        '',
        venueLinks || '- No venues available.',
        '',
      ].join('\n'),
    },
    {
      ...
      content: [
        '# Sports',
        '',
        sportLinks || '- No sports available.',
        '',
      ].join('\n'),
    },
    ...venueFiles,
