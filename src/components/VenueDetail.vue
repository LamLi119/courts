...
const websiteLink = computed(() => socialLinksList().find((link) => link.icon === 'website')?.url || '');
const canGoPrev = computed(() => !!props.hasPrevVenue && !!props.onPrevVenue);
const canGoNext = computed(() => !!props.hasNextVenue && !!props.onNextVenue);
            ...
            <div class="lg:hidden space-y-4 mt-4">
              ...
              <div class="flex flex-col items-end gap-2">
                <button v-if="websiteLink" type="button" :id="`${venue.name}-website-button`"
                    class="btn btn-ghost shrink-0 w-[220px] justify-center"
                ...
                <button type="button" :id="`${venue.name}-WhatsApp-button`" class="btn btn-ghost shrink-0 w-[220px] justify-center"
